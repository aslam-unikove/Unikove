...
//  HealthAssure
//
//  Created by Brijesh Gupta on 29/12/17.
...
public class HASwiftLoader: UIView {
    ...
    class var sharedInstance: HASwiftLoader {
        struct Singleton {
            static let instance = HASwiftLoader(frame: CGRect(x: 0, y: 0, width: Config().size, height: Config().size))
        }
    ...
    public class func show(title: String?, animated : Bool) {
        ...
        let loader = HASwiftLoader.sharedInstance
